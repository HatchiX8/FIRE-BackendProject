...
import { stockInfoMetadataRepository } from './stockInfo.metadata.repository.js';
...
export async function syncStockMetadata(): Promise<{ total: number; synced: number }> {
  // 1) 抓取上市 / 上櫃 HTML
  const [twseHtml, tpexHtml] = await Promise.all([
    fetchHtml(TWSE_ISIN_URL),
    fetchHtml(TPEX_ISIN_URL),
  ]);

  // 2) 解析 HTML 表格為中間結構
  const twseRows = parseIsinHtml(twseHtml, {
    // 上市：要「股票、ETF、TDR」
    allowedSecurityTypes: ['股票', 'ETF', 'TDR'],
  });
  const tpexRows = parseIsinHtml(tpexHtml, {
    // 上櫃：要「股票、ETF」
  ...
  // 5) 清空舊資料，再寫入新資料
  await stockInfoMetadataRepository.deleteAll();

  const BATCH_SIZE = 500;
  for (let i = 0; i < payload.length; i += BATCH_SIZE) {
    const batch = payload.slice(i, i + BATCH_SIZE);
    await stockInfoMetadataRepository.upsertMany(batch);
  }
